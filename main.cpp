...
int main(int argc, char *argv[]) {
  ...
  QString path3;
  path3 = path + "/main.qml";
  QFile::copy(":/main.qml", path3);

  // Register Barcode Scanner Extension Statically
  extern void ring_decode_barcode(void *pPointer);
  RING_API_REGISTER("decode_barcode", ring_decode_barcode);
...
void ringapp_deleteappfiles(void) {
  ...
  ringapp_delete_file(path, "main.qml");
}
...
RING_FUNC(ring_loadlib) {
  ...
  // We don't need loadlib() because ring_qt.cpp is already embedded in the Qt
  // project
}

RING_FUNC(ring_ismobileqt) {
  // A function used by RingQt (Appfile() function) to access files using
  // resources
